...
# the earliest tag whose release actually ships an install.sh; below this we use the current script and do not
# try to re-fetch a per-tag script (it won't exist, e.g. v0.1.0 predates the installer -> 404)
INSTALL_SH_SUPPORTED_VERSION=v0.2.0

# populated at install time with "sudo" when the chosen install dir needs elevated permissions (see set_sudo_prefix)
SUDO=""
...
install_asset() (
  ...
  test ! -d "${destination}" && ${SUDO} install -d "${destination}"

  # install the binary to the destination dir
  ${SUDO} install "${archive_dir}/${binary}" "${destination}/"
)

# dir_on_path [dir]
#
# returns 0 if dir is an entry in $PATH, 1 otherwise.
dir_on_path() {
  case ":${PATH}:" in
    *":$1:"*) return 0 ;;
    *) return 1 ;;
...
# dir_writable [dir]
#
# returns 0 if the current user can create/write into dir (walking up to the nearest existing ancestor
# so a not-yet-created dir counts as writable when its parent is), 1 otherwise.
dir_writable() {
  probe="$1"
  while [ ! -e "$probe" ]; do
    probe=$(dirname "$probe")
...
#
# picks the install dir when the user did not pass -b. we prefer a home-centric dir that is already on
# PATH and writable (so no sudo is ever needed); failing that we fall back to /usr/local/bin, which
# auto-elevation (see set_sudo_prefix) handles being root-owned.
default_install_dir() {
  os="$1"
  if [ "$os" = "windows" ]; then
    echo "./bin"
  ...
  # only reuse a home-centric dir that already exists, is already on PATH, and is writable; we don't
  # create one or assume an absent dir would be picked up by the shell.
  home="${HOME:-}"
  if [ -n "$home" ]; then
    for candidate in "$home/.local/bin" "$home/bin"; do
      if [ -d "$candidate" ] && [ -w "$candidate" ] && dir_on_path "$candidate"; then
        echo "$candidate"
        return 0
      fi
    done
...
# set_sudo_prefix [install-dir]
#
# sets the global SUDO to "sudo" when we cannot write to install-dir ourselves (and are not already root),
# otherwise leaves it empty. this lets `cat install.sh | sh` land a binary in a system dir with no sudo prefix.
set_sudo_prefix() {
  dir="$1"
  SUDO=""

  # already root, or a dir we can write ourselves: nothing to elevate
  if [ "$(id -u)" -eq 0 ] || dir_writable "$dir"; then
    return 0
  fi

  if is_command sudo; then
    SUDO="sudo"
    log_info "install dir '${dir}' needs elevated permissions; using sudo (you may be prompted for your password)"
  else
    log_warn "install dir '${dir}' is not writable and sudo was not found; re-run with -b <writable-dir> or as root"
  fi
}
...
main() (
  ...
  # if unset, a per-platform default is chosen after OS detection (see default_install_dir); an explicit
  # install_dir env var or -b flag always wins.
...
Usage: $0 [-v] [-b DIR] [-d] [TAG]
  -b DIR  the installation directory (defaults to an existing writable dir on your PATH such as
          ~/.local/bin, otherwise /usr/local/bin, elevating with sudo only if needed)
...
EOF
  ...
  if [ "${DOWNLOAD_TAG_INSTALL_SCRIPT}" = "true" ] && compare_semver "${version}" "${INSTALL_SH_SUPPORTED_VERSION}"; then
      export DOWNLOAD_TAG_INSTALL_SCRIPT=false
      log_info "fetching release script for tag='${tag}'"
      install_script=$(http_copy "${INSTALL_SH_BASE_URL}/${tag}/install.sh" "")
      echo "${install_script}" | sh -s -- ${PROGRAM_ARGS}
      exit $?
  fi

  # resolve the per-platform default install dir when none was given, then decide if we need sudo to write there
  if [ -z "${install_dir}" ]; then
      install_dir=$(default_install_dir "${os}")
  fi
  set_sudo_prefix "${install_dir}"
