...
func (m *UI) Setup(subscription partybus.Unsubscribable) error {
	// we still want to collect log messages, however, the logger shouldn't write to the screen directly
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case partybus.Event:
		...
		case event.CLIReportType, event.CLINotificationType, event.CLIExitType:
			...
			// why not return tea.Quit here for exit events? because there may be UI components that still need the
			// update-render loop. for this reason we'll let the bonsai event loop call Teardown() which will
			// explicitly wait for these components.
			return m, nil
		}
...
func postUIEvents(quiet bool, events ...partybus.Event) {
	...
	for _, e := range events {
		...
		if err != nil {
			log.WithFields("error", err).Warnf("failed to gather final report for %q", source)
		} else {
	...
	if !quiet {
		// show all notifications to stderr
		for _, e := range events {
